...
{
	var $player_auto = ""; // 오디오 태그 자동재생 검출용 변수
	...
	function transHTML($xml_obj)
	{
		...
		if($auto_start!="true") $player_auto = "";
		else $player_auto = "autoplay"; // 자동재생여부 확인
		...
		else if(Context::getResponseMethod() != "XMLRPC" && substr ($src,-4) == ".mp4" ) return sprintf("<video class=\"video-js vjs-default-skin\" controls preload=\"none\" width=\"%s\" height=\"%s\" data-setup=\"{}\"><source src=\"%s\" type='video/mp4' /></video>",$width, $height, $src);
		else if(Context::getResponseMethod() != "XMLRPC") return sprintf("<script type=\"text/javascript\">displayMultimedia(\"%s\", \"%s\",\"%s\", { \"autostart\" : %s, \"wmode\" : \"%s\" });</script>", $src, $width, $height, $auto_start, $wmode);
